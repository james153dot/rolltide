#include "C:\Users\james\projects\helloworld\Alabama\NavierStokesInitialConditions.hpp"
/*
...
/* basically stating that must be 2d, flow model type must be single species/5 equation, 2 */
    if (initial_time)
    {
        ...
        if (d_flow_model_type == FLOW_MODEL::SINGLE_SPECIES)
        {
            ...
            for (int j = -num_ghosts_cons_var[1]; j < patch_dims[1] + num_ghosts_cons_var[1]; j++)
            {
                for (int i = -num_ghosts_cons_var[0]; i < patch_dims[0] + num_ghosts_cons_var[0]; i++)
                {
                    ...
                    {
                        u = (u_inf)*erf((x[0]-spongeL)/(D/Real(5)));
                        v = (v_inf)*erf((x[0]-spongeL)/(D/Real(5)));
                        p = half * (p_inf + p_ic) + half * (p_ic - p_inf)*erf((x[0]-spongeL)/(D/Real(5)));
                    }
                    else
                    {
                        u = (u_inf) *erf((x[0]-spongeR)/(D/Real(5)));
                        v = (v_inf) *erf((x[0]-spongeR)/(D/Real(5)));
                        p = half * (p_inf + p_ic) - half * (p_ic - p_inf)*erf((x[0]-spongeR)/(D/Real(5)));
                    }

                    if (x[1] < y_c) // y-values as opposed to x values
                    {
                        u = (u_inf)*erf((x[1]-spongeB)/(D/Real(5)));
                        v = (v_inf)*erf((x[1]-spongeB)/(D/Real(5)));
                        p = half * (p_inf + p_ic) + half * (p_ic - p_inf)*erf((x[1]-spongeB)/(D/Real(5)));
                    }
                    else
                    {
                        u = (u_inf)*erf((x[1]-spongeT)/(D/Real(5)));
                        v = (v_inf)*erf((x[1]-spongeT)/(D/Real(5)));
