    ...
    if (initial_time)
    {
        ...
        if (d_flow_model_type == FLOW_MODEL::SINGLE_SPECIES)
        {
            ...
            for (int k = -num_ghosts_cons_var[2]; k < patch_dims[2] + num_ghosts_cons_var[2]; k++)
            {
                ...
                {
                    for (int i = -num_ghost_cons_var[0]; i < patch_dims[0] + num_ghosts_con_var[0]; i++)
                    
                    {
                        // Compute index into linear data array.
                        int idx_cell = (i + num_ghosts_cons_var[0]) +
                            (j + num_ghosts_cons_var[1])*ghostcell_dims_cons_var[0] + 
                            (k + num_ghosts_con_var[2]) * ghostcell_dims_cons_var[1] * ghostcell_dims_cons_var[0];
                        ...
                        // new velocity terms in x-y-z from AA200 - Applied Aerodynamics - Stanford University
                        // Equation 10.75 from https://web.stanford.edu/~cantwell/AA200_Course_Material/AA200_Lectures/AA200_Ch_10_Elements_of_potential_flow_Vortex_Sticks_and_Stokes_Flow_Brian_Cantwell.pdf
                        {
                            // new velocity terms in x-y-z from AA200 - Applied Aerodynamics - Stanford University
                            // Equation 10.75 from https://web.stanford.edu/~cantwell/AA200_Course_Material/AA200_Lectures/
                        ...
                        */
                        ...
                        /* There will be no discontinuities
